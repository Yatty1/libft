...
/*   Updated: 2018/08/05 21:12:44 by syamada          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cvt_flag_cx(char *conv, va_list ap, char **str, int len)
{
	char	*flag;
	...
	if (!(flag = take_flag(conv, len)))
		return (-1);
	if (ft_strcmp(flag, "hh") == 0)
		*str = ft_uitoa_base((unsigned char)va_arg(ap, unsigned int), 16, 1);
	else if (ft_strcmp(flag, "h") == 0)
		*str = ft_uitoa_base((unsigned short)va_arg(ap, unsigned int), 16, 1);
	else if (ft_strcmp(flag, "l") == 0)
		*str = ft_ultoa_base(va_arg(ap, unsigned long), 16, 1);
	else if (ft_strcmp(flag, "ll") == 0)
		*str = ft_ulltoa_base(va_arg(ap, unsigned long long), 16, 1);
	else if (ft_strcmp(flag, "j") == 0)
		*str = ft_uimaxtoa_base(va_arg(ap, uintmax_t), 16, 1);
	else if (ft_strcmp(flag, "z") == 0)
		*str = ft_sizetoa_base(va_arg(ap, size_t), 16, 1);
	free(flag);
	if (!*str)
		return (-1);
	return (len);
...
static char	*flag_x(char *conv, va_list ap, int is_cap)
{
	...
	{
		if ((len = is_cap ? cvt_flag_cx(conv, ap, &str, len) :
					cvt_flag_x(conv, ap, &str, len)) < 0)
			return (NULL);
	}
	else
